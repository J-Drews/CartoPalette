...
DEFAULT_PIPELINE_CONFIG = {
    ...
    "guardrails": {
        ...
        "min_distinguishability_sequential": 0.30,
        "min_distinguishability_diverging": 0.40,
        "sequential": {
...
class CartoPalette:
    ...
    def _apply_guardrails(self, palettes: list, scheme_type: str, basemap_mean_L: float,
        ...
        min_dist_seq = gc.get("min_distinguishability_sequential", 0.30)
        min_dist_div = gc.get("min_distinguishability_diverging", 0.40)
        min_distinguish = min_dist_div if scheme_type == "diverging" else min_dist_seq
        ...
        for pal in palettes:
            ...
            # Sequential >= 0.30 (9 dE), Diverging >= 0.40 (12 dE)
        ...
        # No fallback here — let suggest() handle retries
        return filtered
    ...
    def suggest(
        ...
        # Extract basemap colors for scoring (once, reused across retries)
        try:
        ...
        # Generate candidates with retry logic (up to 3 rounds if guardrails filter all)
        all_valid = []
        max_rounds = 3

        for _round in range(max_rounds):
            # Generate k candidates (iid from latent space)
            condition_k = condition.repeat(k, 1)
            z = torch.randn(k, self.model.latent_dim, device=self.device)
            raw = self.model.decoder(condition_k, z)
            ...
            filtered = self._apply_guardrails(palettes, scheme, basemap_mean_L, basemap_colors)
            all_valid.extend(filtered)

            # Stop if we have enough valid palettes
            if len(all_valid) >= n_suggestions:
                break

        # If after all rounds still nothing valid, pick the best by distinguishability
        if not all_valid:
            all_scored = [p for p in palettes if p.metrics is not None]
            if all_scored:
                by_dist = sorted(
                    all_scored,
                    key=lambda p: p.metrics.get("distinguishability", 0),
                    reverse=True,
                )
                all_valid = by_dist[:n_suggestions]
            else:
                all_valid = palettes[:n_suggestions]

        # MMR selection for diverse top-n
        selected = self._mmr_select(all_valid, n_suggestions, lambda_mmr)
