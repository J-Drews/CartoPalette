"""CartoPalette v4.1 — Streamlit Web Application."""
...
MODEL_URL = "https://github.com/J-Drews/CartoPalette/releases/download/v4.1/cartopalette_v4.pt"
MODEL_DIR = PROJECT_ROOT / "cartopalette" / "pretrained"
MODEL_PATH = MODEL_DIR / "cartopalette_v4.pt"


def download_model():
    """Download model weights from GitHub Release if not cached locally."""
    if MODEL_PATH.exists():
        return str(MODEL_PATH)
    MODEL_DIR.mkdir(parents=True, exist_ok=True)
    with st.spinner("Downloading model weights (~23 MB)... This only happens once."):
...
def main():
    ...
    with st.sidebar:
        ...
        with st.expander("Advanced", expanded=False):
            n_suggestions = st.slider(
                ...
                help="Number of palette suggestions (default: 3, selected via MMR for diversity)."
            )

            k = st.slider(
                "Candidates (k)",
                min_value=10, max_value=50, value=35,
                help="Number of internal candidates to generate per round. Default k=35 ensures enough diverse palettes pass the quality filters."
            )

            lambda_mmr = st.slider(
                "Diversity (lambda)",
                min_value=0.5, max_value=1.0, value=0.85, step=0.05,
                help="MMR tradeoff: 1.0 = pure quality, lower = more diversity between suggestions. Default 0.85."
            )
        ...
        <strong>About CartoPalette v4.1</strong><br>
        ...
        <strong>Pipeline:</strong> Generate 35 candidates &rarr; Score &rarr; Guardrail filter &rarr; MMR Top-3<br><br>
    ...
    with col_results:
        ...
        # Build a cache key from the current settings + uploaded file
        cache_key = f"{uploaded.name}_{uploaded.size}_{scheme}_{n_classes}_{scale}_{n_suggestions}_{k}_{lambda_mmr}"

        # Only regenerate when settings or image change
        ...
        st.markdown(
            ...
            f'Pipeline: {k if "k" in dir() else 35} candidates &rarr; scored &rarr; '
            f'guardrail-filtered &rarr; MMR top-{len(palettes)} '
            f'(&lambda;={lambda_mmr if "lambda_mmr" in dir() else 0.85})'
            f'</div>',
            unsafe_allow_html=True
        )
