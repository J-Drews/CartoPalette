...
def runtime_uses_v42_inference() -> bool:
    """Return True when the deployed runtime imported the v4.2 inference API."""
    return "reranker" in inspect.signature(CartoPalette.suggest).parameters
...
def download_model():
    """Download the pretrained model on hosted deployments if it is missing."""
    ...
    try:
        ...
        if tmp_path.exists():
            tmp_path.unlink()
        st.warning(f"Could not download pretrained model: {exc}")
        return None
...
def main():
    ...
    with st.sidebar:
        ...
        with st.expander("Deployment Debug", expanded=False):
            st.code(
                "\n".join([
                    f"CartoPalette file: {inspect.getfile(CartoPalette)}",
                    f"suggest signature: {inspect.signature(CartoPalette.suggest)}",
                    f"v4.2 inference loaded: {runtime_uses_v42_inference()}",
                ]),
                language="text",
            )
    ...
    with col_results:
        ...
        with st.spinner("Generating palettes..."):
            try:
                ...
                if not runtime_uses_v42_inference():
                    st.error(
                        "The Streamlit runtime is still importing the old v4.1 "
                        "`cartopalette.core.inference` module. Please reboot or redeploy "
                        "the app so the new v4.2 `CartoPalette.suggest(..., reranker=..., "
                        "apply_repair=...)` API is loaded."
                    )
                    return
                palettes = cp.suggest(
                    image,
